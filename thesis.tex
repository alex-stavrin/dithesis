...
\abstractEn{
This thesis explores the design and development of a Virtual Reality Scene Editor using the Unity game engine. 
...
conducted focusing on task efficiency and user presence. Preliminary results indicate that \emph{Results here...}
}
\abstractGr{
Η πτυχιακή εργασία εξετάζει τη σχεδίαση και ανάπτυξη ενός επεξεργαστή σκηνών σε περιβάλλον Εικονικής Πραγματικότητας 
με τη χρήση της μηχανής παιχνιδιών Unity. Η παραδοσιακή σχεδίαση τρισδιάστατων περιβαλλόντων βασίζεται συνήθως σε 
...
εκτέλεσης εργασιών και στην αίσθηση παρουσίας του χρήστη. Τα προκαταρκτικά αποτελέσματα δείχνουν ότι \emph{Εδώ μπαίνουν τα αποτελέσματα...}
}
\acksEn{
\begin{greek}
...
Game engines like Unity and Unreal Engine utilize traditional WIMP (Windows, Icons, Menus, Pointers) interfaces that
...
Furthermore, as the industry moves toward "Spatial Computing," there is a growing demand for tools that allow for \textbf{in-situ} creation.
...
Ultimately, this thesis explores how moving the "editor" into the "experience" can streamline the creative process and democratize 3D
world-building.
...
In virtual reality (VR), user interfaces are broadly categorized based on their input and target dimensions, 
resulting in a mix of 2D and 3D interaction methods \cite{electronics13030600}.

\subsubsection*{2D Interfaces in VR}
Many VR applications adapt traditional 2D design principles (like Windows, Icons, Menus, and Pointers) into 3D environments.
...
The Unity Editor is the industry standard for XR development, utilizing a \textit{Scene View}.
...
Unreal Engine (UE) represents the high-end of the XR development spectrum, offering robust tools for visual fidelity.
Historically, Epic Games attempted to bridge the 2D-3D gap with the introduction of the \textit{Unreal VR Editor} in 2016.
...
simplification. This deprecation highlights a significant opportunity for specialized, lightweight authoring tools. Unlike the
"feature-heavy" approach of Unreal, \textit{SceneEditorVR} prioritizes a streamlined UX, proving that immersive editing is more
...
Godot Engine offers a unique perspective on the evolution of immersive tools, as it is the only major engine to have
ported its full desktop editor to standalone VR hardware, specifically the Meta Quest series. As of late 2024, the
...
Rec Room represents the most successful implementation of a consumer-facing, VR-native creation system.
...
Arkio is a professional-grade collaborative design tool that bridges the gap between traditional architectural modeling and
...
ShapesXR is an industry-standard prototyping platform designed to facilitate the transition from 2D concepts to 3D spatial interfaces.
...
utilize "Teleportation" to minimize physical discomfort, it was found to be too disjointed or "clunky" for a scene-editing
context where incremental adjustments are frequent. To justify this choice, the application identifies its target audience as
...
\section{Gizmos}
When an asset is selected, one of three possible transformation gizmos can be activated:
...
The scale gizmo has three scalers. One for each axis. The user can interact with the scalers to scale the assets in each axis. For this type
of gizmo there is also a white sphere that when interacted will scale the asset in all of the axis.
...
\begin{itemize}
  ...
  \item \textbf{Redo} : Will redo the last undone action
  ...
  \item \textbf{Undo} : Will undo the last action
\end{itemize}
...
The level editor features a central control panel modeled after the Meta Horizon OS interface. This panel can be freely
...
The Scene tab provides essential management tools, allowing users to save their progress, export the
...
They were provided with an information sheet detailing the research objectives

\section{Consent Form}
Next the participants were asked to sign a consent form.

\section{Participant Information Questionnaire}
A questionnaire was given to the participants to gather demographic data. [INSERT QUESTIONS HERE]

\section{Familiarization with the hardware}
Participants were equipped with a Meta Quest headset and given a brief explanation of the physical boundaries 
(room-scale setup) and the basic controller layout to ensure they felt comfortable and safe before entering the virtual environment.

\section{Recording}
A recording of the user's view is iniated to gather information about his/her experience.

\section{Guided Tutorial with specific Tasks}
...
During this stage, participants were asked to complete a series of specific, progressively complex tasks. These tasks included:
\begin{itemize}
    \item Creating a new level/scene and giving a name.
    \item Fill rest...
\end{itemize}
...
\subsection{VRSUQ}
To evaluate the system's usability, the Virtual Reality System Usability Questionnaire (VRSUQ) was administered.
...
\subsection{UEQ-S}
The short version of the User Experience Questionnaire (UEQ-S) was utilized to measure
...
In addition to the standardized forms, a set of custom Likert-scale questions was designed specifically for this study. 
...
\subsection{Custom Questions}
Participants were asked open-ended questions regarding their workflow, such as: 
"How did the VR editing experience compare to your prior experiences with desktop editors like Unity or Godot?" 
and "Which tools did you find most or least intuitive?"

\subsection{Positives}
Users were encouraged to highlight the aspects of the system they found most successful. 
This section of the interview aimed to validate the core design choices, such as the effectiveness
of the radial menus for quick actions and the natural feel of the 6DoF gizmo manipulation.

\subsection{Negatives}
Finally, participants discussed the limitations and pain points they encountered. Gathering constructive criticism on 
elements like UI placement, snapping sensitivity, or physical fatigue (e.g., "gorilla arm" syndrome) is critical for 
identifying areas of improvement for future iterations of the software.
